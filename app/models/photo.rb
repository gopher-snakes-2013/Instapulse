...
#Want keys saved as symbols?  (update test line 42 if so)
	...
	def self.create_tuples(playback_attributes)
		...
		until iterate == false
			...
			batch_B_complete_objects = Photo.where(created_time: ((time_keeper+(batch_time/10)))..(time_keeper + batch_time + (batch_time/10)))
			batch_B_digested = Photo.grab_media_info(batch_B_complete_objects)
			tuple << batch_B_digested

			output << tuple

			time_keeper += (batch_time - (batch_time/1.25))
